...
当手键用，软件按你实际拍出的长短识别点划，逐字判定，拍错就原地重发。

- 零运行时依赖：HTML + CSS + ES Module，直接部署成 GitHub Pages 就能用
- 自适应时序：不写死“小于 80ms 算点”，而是按这次拍出的时长反推你的点长
- 判定只有一个来源（`src/core/practice.ts`）：按键不判定，停顿才判定，拍错不跳字
- 课程内容都是有意义的报文/词组/缩略语，每条都逐词标了直译（连发练习没有词义，写一句说明），不是随机字母`r`n- 报文里的呼号都走占位符：自己的从设置里取，对方是一位虚构的固定呼号
- 练习页像歌词一样摆：当前这句按词排开、词下写直译，上下各留一条相邻条目淡出
- 输入就一个鼠标左键；四个过程信号可以直接用手键操作界面，全程不用摸鼠标
...
直键练习器在系统里就是一个鼠标左键，所以练习页中间那一大块就是手键：按住是按下，松开是抬起。

想干别的也不用离开手键：底部五个动作都能用过程信号拍出来，按钮上除了信号名，
还画着它的码形，照着拍就行。
...
目标字优先：课上正好在练 `?` 时，它就是目标字，不会被当成命令。反过来，
目标字同时又是某个过程信号的开头时（比如目标是 E、你想发 AR），
程序等你停一下再定：停下来就是 E，接着往下发就是 AR。
...
它会检查：页面不是空白、无 JS 报错、说明页列全了过程信号、进页面就是拍发状态、**判对的字是不是真的变绿**（读 computed style 比对 rgb 值）、按住时发报条是不是真的长出来、拍错是不是真的给红问号、发 AR 换下一条 / 发 BK 回上一条、成绩弹窗有成绩、报文里能看出分词，并留下 13 张截图。
...
src/core/             ← 与 UI 完全解耦，可在 Node 里直接测
  ...
  lessons.ts          课程内容（词组、缩略语、QSO；每条带句意与逐词直译）
  audio.ts            侧音 + 参考发送（Web Audio 调度）
  settings.ts         设置与进度的 localStorage 持久化
  util.ts             小工具
src/ui/               ← 视图与输入
  input.ts            鼠标左键 → 带精确时长的按下/抬起事件
...
- 字与字之间靠静音分：引擎在静音 2.5 个单位时就断字（标准是 3），词间隔（7）只作为提示，不参与判定。`r`n- `=`（BT）只在正式报文里分段用；日常通联的课文里靠词间停顿，不发这个符号。
