...
打开页面后：**课程**里选一课，或者直接进**练习** —— 进来就能拍，没有“开始”这一步。
鼠标左键/右键、键盘 `空格` `J` `K` `回车` 都可以当直键。

想干别的也不用离开手键：底部四个动作都能用过程信号拍出来。
...
| `pnpm test:browser` | 用 CDP 驱动无头 Chrome 打开页面，真点按钮、真按空格拍键，截图并断言（28 项） |
...
它会检查：页面不是空白、无 JS 报错、进页面就是拍发状态、**判对的字是不是真的变绿**（读 computed
style 比对 rgb 值）、按住时发报条是不是真的长出来、拍错是不是真的给红问号、
发 AR 能不能直接换下一条、成绩弹窗有成绩、报文里能看出分词，并留下 13 张截图
（帮助/练习/透镜/拍发/按住/拍对/拍错/结算/课程/长报文/AR 换条/设置/统计）。
